"""Calibrated benchmark for Standard and three StageKV execution paths.

This is a measurement script, not a new algorithm.  It compares a standard
GPU DynamicCache against the Day-7 synchronous path, Day-8 H2D-only async
path, and Day-10 bidirectional H2D/D2H async path under one protocol:
...
RESULTS_DIR = "/root/stagekv/results/day11_bidirectional_calibrated"
DEFAULT_LENGTHS = (4096,)
DEFAULT_RESIDENT_HEADS = (0, 1, 2, 4)
MARKER = "__STAGEKV_DAY11_RESULT__="
...
class MethodSpec:
    ...
    def async_enabled(self) -> bool:
        return self.family in {"async", "bidirectional"}
...
def new_runtime(
    ...
    *,
    ...
    DynamicCache | StaticPinnedResidentCache | DeferredAsyncResidentCache,
    PhaseAwareOffloadPatch
    | AsyncDoubleBufferPatch
    | BidirectionalAsyncPatch
    | None,
]:
    ...
    if spec.family == "bidirectional":
        ...
        patch_type = BidirectionalAsyncPatch
    else:
        ...
        patch_type = (
            AsyncDoubleBufferPatch
            if spec.family == "async"
            else PhaseAwareOffloadPatch
        )
    patch = patch_type(
        model,
        cache,
...
def timed_model_forward(
    ...
    *,
    ...
    cache: DynamicCache | StaticPinnedResidentCache | DeferredAsyncResidentCache,
...
def cache_statistics(
    ...
    bidirectional_patch = (
        patch if isinstance(patch, BidirectionalAsyncPatch) else None
    )
    staging_bytes = 0
    if async_patch is not None:
        staging_bytes = async_patch.staging_allocated_bytes
    elif bidirectional_patch is not None:
    ...
    return {
        ...
        "async_prefetch_groups": (
            async_patch.async_prefetch_group_calls
            if async_patch is not None
            else (
        ...
        "h2d_event_timing_available": async_patch is not None,
        "dedicated_transfer_stream_enabled": (
            (async_patch is not None and async_patch.transfer_stream is not None)
            or (
...
def comparison_rows(summary: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for length in lengths:
        ...
        for resident in {row["resident_kv_heads_r"] for row in summary}:
            ...
            if sync is not None and async_row is not None:
                output.append(
                    {
            ...
            if async_row is not None and bidirectional is not None:
                output.append(
                    {
                        "comparison": "bidirectional_vs_async_same_residency",
                        "sequence_length": length,
                        "candidate_method": bidirectional["method"],
                        "reference_method": async_row["method"],
                        "resident_kv_heads_r": resident,
                        "prefill_cuda_change_percent": (
                        ...
                        * 100.0,
                        "bidirectional_over_async_steady_speed_ratio": (
                            async_row["decode_steady_cuda_mean_ms_mean"]
                            / bidirectional["decode_steady_cuda_mean_ms_mean"]
                        ),
                        "async_h2d_gib": async_row["h2d_gib_mean"],
                        "bidirectional_h2d_gib": bidirectional["h2d_gib_mean"],
                        "bidirectional_d2h_gib": bidirectional["d2h_gib_mean"],
                        "bidirectional_blocking_d2h_copies": bidirectional[
                            "blocking_d2h_tensor_copies_mean"
                        ],
                        "same_persistent_gpu_kv_gib": math.isclose(
                            async_row["cache_gpu_gib_mean"],
...
def save_results(
    ...
    write_csv(results_dir / "day11_warmup.csv", warmups)
    write_csv(results_dir / "day11_raw.csv", rows)
    write_csv(results_dir / "day11_per_token.csv", per_token_rows(rows))
    write_csv(results_dir / "day11_summary.csv", summary)
    write_csv(results_dir / "day11_comparison.csv", comparison)
    manifest = {
        "benchmark": "StageKV bidirectional calibrated benchmark",
        ...
        "files": {
            "warmup": "day11_warmup.csv",
            "raw": "day11_raw.csv",
            "per_token": "day11_per_token.csv",
            "summary": "day11_summary.csv",
            "comparison": "day11_comparison.csv",
        },
    }
    (results_dir / "day11_manifest.json").write_text(
...
def orchestrate(args: argparse.Namespace) -> int:
    ...
    for length in args.lengths:
        print(f"\n=== Bidirectional calibrated sequence_length={length} ===", flush=True)
        command = [
            sys.executable,
    ...
    save_results(results_dir, rows, warmups, specs, args)
    print(f"warmup={results_dir / 'day11_warmup.csv'}")
    print(f"raw={results_dir / 'day11_raw.csv'}")
    print(f"per_token={results_dir / 'day11_per_token.csv'}")
    print(f"summary={results_dir / 'day11_summary.csv'}")
    print(f"comparison={results_dir / 'day11_comparison.csv'}")
    print(f"manifest={results_dir / 'day11_manifest.json'}")
    print("stagekv_bidirectional_calibrated_benchmark=PASS")
    return 0
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--stagekv-modes",
        choices=("sync", "async", "bidirectional"),
        nargs="+",
        default=("sync", "async", "bidirectional"),
    )
    parser.add_argument(
        "--smoke",
        action="store_true",
